...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # get the already eaten ghost's position
        j = self.getJailPosition()
        # checking special case where noisy distance is None as mentioned above
        if noisyDistance is None :
            # update the belief of already eaten ghost
            allPossible[j] = 1.0
        else:
            # if noisy distance is available get all the legal positions of ghosts
            for gLegalPos in self.legalPositions:
                # calculate the manhattan distace to each of the legal ghost positions
                dist = abs(util.manhattanDistance(gLegalPos, pacmanPosition))
                # update the possible next state with belief probability and given distance probability
                allPossible[gLegalPos] = self.beliefs[gLegalPos] * emissionModel[dist]
        # normalize the values so that they add up to 1
        allPossible.normalize()
        # update the beliefs with all possible transitions computed above 
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # looping through the current legal positions for ghosts and \
        # based on the probabilities returned for each move for the ghost \
        # from the current legal position, update the probability of the \
        # new position with the probability returned multiplied with the belief \
        # of the current position
        for gLegalPos in self.legalPositions:
            newPosDist = self.getPositionDistribution( self.setGhostPosition(gameState, gLegalPos)).items()
            for newPos, prob in newPosDist:
                allPossible[newPos] += prob * self.beliefs[gLegalPos]
        # update the beliefs of positions with the new calculated values
        self.beliefs = allPossible
